...
class SqliteDB(
    ...
    private fun makeMock() {
        ...
        for (i in 0..20) {
            val place = Place("카페$i", "복현${i}동", PlaceCategory.CAFE)
            insertPlace(place)
        }
        for (i in 0..20) {
            val place = Place("식당$i", "두류${i}동", PlaceCategory.RESTAURANT)
            insertPlace(place)
        }
        for (i in 0..20) {
            val place = Place("약국$i", "카카오${i}동", PlaceCategory.PHARMACY)
            insertPlace(place)
        }
        for (i in 0..20) {
            val place = Place("기타$i", "쿠키즈${i}동", PlaceCategory.ELSE)
            insertPlace(place)
        }
    }
